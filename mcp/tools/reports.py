...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .report-body h1, .report-body h2 {{ font-size: 1.25rem; font-weight: 700; color: var(--blue);
                      margin: 2rem 0 1rem; padding-bottom: 0.5rem;
                      border-bottom: 2px solid var(--border); }}
  .report-body h3 {{ font-size: 1rem; font-weight: 700; color: var(--dark); margin: 1.25rem 0 0.5rem; }}
  .report-body p {{ line-height: 1.8; color: #475569; margin-bottom: 0.75rem; }}
  .report-body ul, .report-body ol {{ line-height: 1.8; color: #475569; margin: 0.5rem 0 0.75rem 1.5rem; }}
  .report-body pre {{ background: #1e293b; color: #7dd3fc; font-family: monospace;
    font-size: 0.8rem; padding: 1rem 1.25rem; border-radius: 0.5rem;
    overflow-x: auto; margin: 0.75rem 0; line-height: 1.6; white-space: pre; }}
  .report-body code {{ font-family: monospace; background: #f1f5f9;
  ...
  .report-body th {{ background: var(--blue); color: white; padding: 0.5rem 0.75rem; text-align: left; }}
  .report-body td {{ padding: 0.4rem 0.75rem; border-bottom: 1px solid var(--border); }}
...
def create_html_report(
    ...
    try:
        ...
        # Fallback: try to repair common LLM JSON mistakes (invalid backslash escapes)
        import re as _re
        fixed = _re.sub(r'\\(?!["\\/bfnrtu])', r'\\\\', sections_json)
